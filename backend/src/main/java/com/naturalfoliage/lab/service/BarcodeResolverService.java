...
public class BarcodeResolverService {
    ...
    public Optional<Subculture> findSubculture(String scannedValue) {
        return subcultures.findByBarcode(scannedValue.trim());
    }
    ...
    private Optional<Long> decodeMotherId(String value) {
        ...
        if (!normalized.matches(MOTHER_PREFIX + "\\d{8}")) return Optional.empty();
        return Optional.of(Long.parseLong(normalized.substring(MOTHER_PREFIX.length())));
    }
}
